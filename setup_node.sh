#!/bin/bash
# ============================================================
# LAB 3 DHCP - NODE VM (VM 3) - Pastebin pipe-ready
...
echo -e "${GREEN}=== LAB 3 DHCP - NODE VM SETUP ===${NC}"
echo "DHCP client | Expected IP 192.168.99.82 from Relay Kea (192.168.99.81)"
echo "REQUIRES Gateway AND Relay running first"
echo ""

# Detect bridged interface (Node has only Adapter 3 active)
...
if echo "$NODE_IP" | grep -q "192.168.99"; then
...
ping -c2 192.168.99.81 > /dev/null 2>&1 && echo -e "  ${GREEN}OK${NC}  ping 192.168.99.81 (Relay)" || echo -e "  ${RED}FAIL${NC} ping 192.168.99.81 (Relay)"
ping -c2 192.168.99.1  > /dev/null 2>&1 && echo -e "  ${GREEN}OK${NC}  ping 192.168.99.1  (Gateway)" || echo -e "  ${RED}FAIL${NC} ping 192.168.99.1  (Gateway)"
ping -c2 192.168.99.2  > /dev/null 2>&1 && echo -e "  ${GREEN}OK${NC}  ping 192.168.99.2  (Relay enp0s8)" || echo -e "  ${RED}FAIL${NC} ping 192.168.99.2  (Relay enp0s8)"

echo ""
echo -e "${GREEN}=== NODE VM SETUP COMPLETE ===${NC}"
echo -e "  $BRIDGE_IF = $NODE_IP (dynamic from Relay Kea 192.168.99.81)"
